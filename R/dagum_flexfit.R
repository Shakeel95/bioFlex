#' Fitting Dagum distribution via maximum likelihood
#'
#'dagum_flexfit is used to fit a Dagum distribution to a strictly positive response variable. The scale parameter may be specified either as a function of covariates or as a constant estimated using the response variable alone.
...
dagum_flexfit <- function(formula, data = NULL, weights = NULL, subset = NULL, ownstart = NULL, key = TRUE, warnings = FALSE, warnings1 = FALSE, ...) {
  ...
  list_start1 <- list(a = fisk_shape(data[,1]), b = fisk_scale(data[,1]), p = 1)

  # define log-likelihood
  LL1 <- function(b, a, p) {
    R = dgb2(data[,1], scale = b, shape1 = a, shape2 = p, shape3 = 1)
    -sum(log(R))
  }
  ...
  dat <- data
  cons <- (gamma(p1 + a1^{-1})*gamma(1 - a1^{-1}))/gamma(p1)
  ...
  if (isTRUE("(Intercept)" %in% names(coefficients(lm_start)))) {
    ...
    # build list of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(a = a1, p = p1, Intercept = coefficients(lm_start)[1])
    ...
    # define log-likelihood
    LL <- function(a, p, Intercept, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      mu = 0
      for (i in 2:ncol(data)) {
        mu = mu + data[,i]*get(paste0("beta",i-1))
      }
      mu <- mu + Intercept
      R = ddagum(data[,1], scale = exp(mu), shape1.a = a, shape2.p = p, log = TRUE)
      -sum(R)
    }
    ...
    # build list of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(a = a1, b = b1, p = p1)
    } else {
      list_start <- ownstart
    }
    ...
    # define log-likelihood
    LL <- function(a, b, p) {
      R = ddagum(data[,1], scale = b, shape1.a = a, shape2.p = p, log = TRUE)
      -sum(R)
    }
    ...
    # build lis of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(a = a1, p = p1)
      for (i in 2:ncol(data)) {
    ...
    # define log-likelihood
    LL <- function(a, p, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      mu = 0
      for (i in 2:ncol(data)) {
        mu = mu + data[,i]*get(paste0("beta",i-1))
      }
