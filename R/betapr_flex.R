#' Fitting Beta Prime distribution via maximum likelihood
#'
#'betapr_flexfit is used to fit a Beta Prime distribution to a strictly positive response variable. The shape parameter may be specified either as a function of covariates or as a constant estimated using the response variable alone.
...
#'@param data An optional data frame, list or environment (or object coercible by as.data.frame to a data frame) containing the variables in the model. If not found in data, the variables are taken from environment(formula), typically the environment from which betapr_flexfit is called.
...
betapr_flexfit <- function(formula, data = NULL, weights = NULL, subset = NULL, ownstart = NULL, key = TRUE, warnings = FALSE, ...) {
  ...
  dat <- data
  dat[,1] <- log(dat[,1]*(betapr_shape2(dat[,1])-1))
  ...
  if (isTRUE("(Intercept)" %in% names(coefficients(lm_start)))) {
    ...
    if (is.null(ownstart)) {
      list_start <- list(beta = betapr_shape2(data[,1]), Intercept = coefficients(lm_start)[1])
    ...
    LL <- function(beta, Intercept, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      mu = 0
      for (i in 2:ncol(data)) {
        mu = mu + data[,i]*get(paste0("beta",i-1))
      }
      mu <- mu + Intercept
      R = dbetapr(data[,1], shape1 = exp(mu), shape2 = beta, log = TRUE)
      -sum(R)
    }
    ...
    if (isTRUE(key)) {
      Parameters <- c("beta", "Intercept")
      Equals <- c("->","->")
      Interpretation <- c("second shape parameter", "intercept in linear predictor")
      for (i in 2:length(coefficients(lm_start))) {
    ...
    # build list of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(alpha = betapr_shape1(data[,1]), beta = betapr_shape2(data[,1]))
    } else {
      list_start <- ownstart
    }
    ...
    LL <- function(alpha, beta) {
      R = dbetapr(data[,1], shape1 = alpha, shape2 = beta, log = TRUE)
      -sum(R)
    }
    ...
    # build list of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(beta = betapr_shape2(data[,1]))
    ...
    # define log-likelihood
    LL <- function(beta, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      mu = 0
      for (i in 2:ncol(data)) {
        mu = mu + data[,i]*get(paste0("beta",i-1))
      }
    ...
    if (isTRUE(key)) {
      Parameters <- c("beta")
      Equals <- c("->")
      Interpretation <- c("second shape parameter")
      for (i in 1:length(coefficients(lm_start))) {
        Parameters <- c(Parameters, names(list_start)[i+1])
        Equals <- c(Equals, "->")
        Interpretation <- c(Interpretation, paste(names(coefficients(lm_start))[i], " coefficient in linear predictor"))
