...
#'@details This function uses the same parametrization of the Fisk distribution as is used in Kleiber and Kotz (2003). Starting values for the maximum likelihood estimation procedure are obtained by solving for the roots of a trigonometric equation equal to the ratio of the first and second moment. The probability probability density function is used is:
...
fisk_flexfit <- function(formula, data = NULL, weights = NULL, subset = NULL, ownstart = NULL, key = TRUE, warnings = FALSE, ...) {
  ...
  dat <- data
  const <- (pi/fisk_shape(dat[,1]))/(sin(pi/fisk_shape(dat[,1])))
  ...
  if (isTRUE("(Intercept)" %in% names(coefficients(lm_start)))) {
    ...
    if(is.null(ownstart)) {
      list_start <- list(beta = fisk_shape(data[,1]), Intercept = coefficients(lm_start)[1])
    ...
    LL <- function(beta, Intercept, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      ...
      mu <- mu + Intercept
      R = dllogis(data[,1], shape = beta, scale = exp(mu), log = TRUE)
      -sum(R)
    }
    ...
    # build list of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(alpha = fisk_scale(data[,1]), beta = fisk_shape(data[,1]))
    } else {
      list_start <- ownstart
    }
    ...
    LL <- function(alpha, beta) {
      R = dllogis(data[,1], scale = alpha, shape = beta, log = TRUE)
      -sum(R)
    }
    ...
    # build list of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(beta = fisk_shape(data[,1]))
