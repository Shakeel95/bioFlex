#' Fitting Lomax distribution via maximum likelihood
#'
#'lomax_flexfit is used to fit a Lomax distribution to a strictly positive response variable. The scale parameter may be specified either as a function of covariates or as a constant estimated using the response variable alone.
...
#'@param data An optional data frame, list or environment (or object coercible by as.data.frame to a data frame) containing the variables in the model. If not found in data, the variables are taken from environment(formula), typically the environment from which lomax_flexfit is called.
...
#'@param ownstart An optional list containing starting values for the maximum likelihood estimation procedure. If a model with an intercept has been specified, the list must be of the form ownstart = list(alpha = , beta0 = , beta1 =, …); if a mode with no intercept has been specified the list must be of the form ownstart = list(alpha = , beta1 = , …); if the rate parameter is not a function of covariates the list must be of the form ownstart = list(alpha = , lambda = ). It is important that the list have as many elements as there are parameters in the model, and that these be supplied in the order set out above.
...
#'@details λ = exp(β1x1 + …. + βkxk)
#'@details When a null model is specified (formula = y ~ 0) λ is not estimated as a function of covariates. The starting value for the maximum likelihood estimation procedure is obtained by calling the function lomax_scale.
#'@references Kempthorne. "Parameter Estimation Fitting Probability Distributions Method Of Moments." MIT 18.443 (2015)
#'@export

lomax_flexfit <- function(formula, data = NULL, weights = NULL, subset = NULL, ownstart = NULL, key = TRUE, warnings = FALSE, ...) {
  ...
  dat <- data
  dat[,1] <- log((lomax_shape(dat[,1]) - 1)*dat[,1])
  ...
  if (isTRUE("(Intercept)" %in% names(coefficients(lm_start)))) {
    ...
    if(is.null(ownstart)) {
      list_start <- list(alpha = lomax_shape(data[,1]), Intercept = coefficients(lm_start)[1])
    ...
    LL <- function(alpha, Intercept, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      mu = 0
      for (i in 2:ncol(data)) {
        mu = mu + data[,i]*get(paste0("beta",i-1))
      }
      mu <- mu + Intercept
      R = dLomax(data[,1], scale = exp(mu), shape = alpha)
      -sum(log(R))
    }
    ...
    # build list of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(alpha = lomax_shape(data[,1]), lambda = lomax_scale(data[,1]))
    } else {
      list_start <- ownstart
    }
    ...
    LL <- function(alpha, lambda) {
      R = dLomax(data[,1], scale = lambda, shape = alpha)
      -sum(log(R))
    }
    ...
    # build list of starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(alpha = lomax_shape(data[,1]))
    ...
    # define log-likelihood
    LL <- function(alpha, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      mu = 0
      for (i in 2:ncol(data)) {
        mu = mu + data[,i]*get(paste0("beta",i-1))
      }
