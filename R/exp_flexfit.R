#' Fitting Exponential distribution via maximum likelihood
#'
#'exp_flexfit is used to fit an exponential distribution to a strictly positive response variable. The rate parameter may be specified either as a function of covariates or as a constant estimated using the response variable alone.
#'If the rate parameter is specified to be a function of covariates, the canonical log link function is used.  
#'@param formula An object of class "formula" (or one that can be coerced to that class): a symbolic description of the model to be fitted.
#'@param data An optional data frame, list or environment (or object coercible by as.data.frame to a data frame) containing the variables in the model. If not found in data, the variables are taken from environment(formula), typically the environment from which exp_flexfit is called.
...
#'@details This function uses the most common parametrization of the Exponential distribution. The starting value for the maximum likelihood estimation procedure is given by the canonical estimator for the rate parameter in an Exponentially distributed random variable: the inverse of the sample mean. The probability probability density function is used is: 
#'@details f(y) = λexp(-λy)
#'@details When the argument formula specifies a full model with an intercept, the rate parameter takes the following form and is estimated via a two step (least squares and maximum likelihood) procedure:
...
#'@details When a null model is specified (formula = y ~ 0) the rate parameter is not estimated as a function of covariates. The starting value for the maximum likelihood estimation procedure is obtained by calling the function exp_rate. 
#'@references 
#'@export 
#'@export

exp_flexfit <- function(formula, data = NULL, weights = NULL, subset = NULL, ownstart = NULL, key = TRUE, warnings = FALSE, ...) {
  ...
  if(ncol(data) > 31)stop("At this time exp_fit() accepts at most 30 covariates")
  ...
  if (isTRUE("(Intercept)" %in% names(coefficients(lm_start)))) {
    ...
    # build starting values for mle2
    if (is.null(ownstart)) {
      list_start <- list(Intercept = coefficients(lm_start)[1])
      for (i in 2:length(coefficients(lm_start))) {
        list_start[i] <- coefficients(lm_start)[i]
      }
      for (i in 2:length(coefficients(lm_start))) {
        names(list_start)[i] <- paste("beta", i-1, sep = "")
      }
    } else {
      list_start <- ownstart
    }
    ...
    LL <- function(Intercept, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      mu = 0
      for (i in 2:ncol(data)) {
        mu = mu + data[,i]*get(paste0("beta",i-1))
      }
      mu = Intercept + mu
      R = dexp(data[,1], rate = exp(mu), log = TRUE)
      -sum(R)
    }
    ...
    if(isTRUE(key)) {
      ...
      Equals <- c("->")
      Interpretation <- c("intercept in linear predictor")
      for (i in 2:length(coefficients(lm_start))) {
        Parameters <- c(Parameters, names(list_start)[i])
        Equals <- c(Equals, "->")
    ...
    if (is.null(ownstart)) {
      list_start <- list(lambda = exp_rate(data[,1]))
    } else {
      list_start <- ownstart
    }
    ...
    # define log-likelihood function
    LL <- function(lambda) {
      R = dexp(data[,1], rate = lambda, log = TRUE)
      -sum(R)
    }
    ...
    if (is.null(ownstart)) {
      ...
      for (i in  1:length(coefficients(lm_start))) {
        names(list_start)[i] <- paste("beta", i, sep = "")
      }
    } else {
      list_start <- ownstart
    }
    ...
    LL <- function(beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, beta7 = 0, beta8 = 0, beta9 = 0, beta10 = 0, beta11 = 0, beta12 = 0, beta13 = 0, beta14 = 0, beta15 = 0, beta16 = 0, beta17 = 0, beta18 = 0, beta19 = 0, beta20 = 0, beta21 = 0, beta22 = 0, beta23 = 0, beta24 = 0, beta25 = 0, beta26 = 0, beta27 = 0, beta28 = 0, beta29 = 0, beta30 = 0) {
      mu = 0
      for (i in 2:ncol(data)) {
        mu = mu + data[,i]*get(paste0("beta",i-1))
      }
